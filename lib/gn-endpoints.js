...
function GnEndpoints(options, constants) {
	...
	try {
		...
		console.log('FALHA AO LER O CERTIFICADO')
		console.log(error);
	}
}
...
GnEndpoints.prototype.run = function (name, params, body) {
	...
	if (self.constants.ENDPOINTS.PIX.hasOwnProperty(name)) {
		...
		this.options.baseUrl = this.options.sandbox ? constants.URL.PIX.sandbox : this.constants.URL.PIX.production;
	}else{
		self.endpoint = self.constants.ENDPOINTS.DEFAULT[name];
		this.options.baseUrl = this.options.sandbox ? constants.URL.DEFAULT.sandbox : this.constants.URL.DEFAULT.production;
	}
	self.body = body;
	self.params = params;
